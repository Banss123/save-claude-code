-- 매장별 체크리스트·리뷰 시트 링크 (사용자 비전: 가장 많이 쓰는 링크 = 첫 줄 강조)

alter table public.stores
  add column if not exists checklist_sheet_url text,
  add column if not exists review_sheet_url text;

comment on column public.stores.checklist_sheet_url is
  '매장별 체크리스트 시트 (영업자가 가장 자주 여는 시트)';
comment on column public.stores.review_sheet_url is
  '매장별 리뷰 작성·점검 시트';
...
select
  ...
  s.main_keywords_i18n,
  s.naver_place_url,
  s.google_map_url,
  s.drive_folder_url,
  s.onboarding_sheet_url,
  s.checklist_sheet_url,                         -- NEW
  s.review_sheet_url,                            -- NEW
...
comment on view public.v_store_360 is
  'v4: checklist_sheet_url + review_sheet_url 추가. 6개 바로가기 링크 지원.';
