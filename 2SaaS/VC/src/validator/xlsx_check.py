...
def validate_xlsx(plan: MenuPlan, xlsx_path: str | Path) -> CheckGroup:
    ...
    # 신규 추가 그룹(한그릇·1인 진입, [신규]) 메뉴는 정상적인 추가이므로 WARN 제외.
    # 가안 그룹명이 "[한그릇·1인]" 또는 "[신규]" 로 시작하면 그 그룹 아이템 수를 "정당한 추가분" 으로 본다.
    added_menu_count = 0
    added_group_names: list[str] = []
    for g in plan.proposed.groups:
        if g.name.startswith("[한그릇·1인]") or g.name.startswith("[신규]"):
            added_menu_count += len(g.items)
            added_group_names.append(g.name)

    menu_count_match = (json_cur_count == json_pro_count)
    # 추가 그룹이 있으면 가안 = 현안 + 추가 메뉴 수 허용
    expected_pro_with_additions = json_cur_count + added_menu_count
    legitimate_addition = (
        added_menu_count > 0 and json_pro_count == expected_pro_with_additions
    )

    if menu_count_match:
        status = CheckStatus.PASS
        msg = f"현안 {json_cur_count}건 == 가안 {json_pro_count}건"
    elif legitimate_addition:
        status = CheckStatus.PASS
        msg = (
            f"현안 {json_cur_count}건 + 신규 그룹 {added_menu_count}건 = "
            f"가안 {json_pro_count}건 (추가 그룹: {', '.join(added_group_names)})"
        )
    else:
        status = CheckStatus.WARN
        msg = (
            f"현안 {json_cur_count}건 ≠ 가안 {json_pro_count}건 "
            f"(추가/삭제 확인 필요; 신규 그룹 추가분 {added_menu_count}건)"
        )
    items.append(CheckItem(
        name="현안/가안 메뉴 수 동일",
        status=status,
