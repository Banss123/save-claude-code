...
class TierTarget(BaseModel):
    """단일 tier 도달 계획 (3/6/12개월 중 하나).

    target_revenue.py 가 산출하는 원본 구조와 1:1 매핑되는 검증 레이어.
    season_factor는 업종·월별 보정치(기본 1.0)로, rationale 노출 시 포함.
    """

    month: int = Field(
        ge=1, le=12, description="도달 목표 월 (1~12, 현재월 + N개월 계산 후)"
    )
    multiplier: float = Field(
        gt=0, description="현재 매출 대비 배수 (예: 1.6 = +60%)"
    )
    season_factor: float = Field(
        default=1.0, gt=0, description="업종별 월별 시즌 팩터"
    )
    target: int = Field(ge=0, description="해당 tier 도달 목표 매출 (원)")


class TierPlan(BaseModel):
    """3/6/12개월 tier 분할 목표 (A/B/C 케이스만 존재).

    D(운영 정비)·E(신규·소형)는 이 필드 자체가 None 유지.
    """

    tier1_3m: TierTarget = Field(description="3개월차 진입 목표")
    tier2_6m: TierTarget = Field(description="6개월차 중간 목표")
    tier3_12m: TierTarget = Field(description="12개월차 최종 스트레치")


class TamMeta(BaseModel):
    """상권 TAM 조회 결과 요약 (근거 섹션 표시용).

    available=False 이면 상권 데이터 미연동 상태. 호출자는 기존 4.0배 캡 폴백.
    정확한 스키마는 src/market/tam_estimator.py::TamEstimate 를 따르며,
    docx 표시에 필요한 최소 필드만 복제한다.
    """

    available: bool = Field(description="TAM 데이터 연동 가능 여부")
    reason: str | None = Field(
        default=None, description="미연동 사유 (available=False 시 필수)"
    )
    tam_monthly_revenue_won: int | None = Field(
        default=None, description="월 기준 상권 TAM 추정액 (원)"
    )
    commercial_zone_id: str | None = Field(
        default=None, description="상권 구역 ID (SHP 기반)"
    )


class TargetMeta(BaseModel):
    """목표 매출 산정 근거 메타 (docx 근거 섹션 표시용).

    solution_builder._meta 가 리턴하는 dict 중 근거 섹션에 직접 쓰이는 필드만 선별.
    검증은 optional — 예전 세션 JSON 과 호환 위해 모든 필드 기본값 허용.
    """

    target_case: Literal["A", "B", "C", "D", "E", "manual", "LEVER"] | None = Field(
        default=None,
        description="케이스 분류 (A~E / manual / LEVER 정본 4-레버)",
    )
    target_case_label: str | None = Field(
        default=None,
        description="케이스 한국어 라벨 (예: 성장 잠재력 큼 (스트레치))",
    )
    target_rationale: str | None = Field(
        default=None, description="산정 근거 문장 (KPI sub_label 원문)"
    )
    repeat_pct: float | None = Field(
        default=None, ge=0, le=100, description="재주문률 % (참고)"
    )
    repeat_diff_pp: float | None = Field(
        default=None, description="업종 평균 대비 재주문률 차이 (%p)"
    )
    historical_override_used: bool = Field(
        default=False,
        description="과거 실적 분포(P50/P80) 오버라이드 적용 여부",
    )
    lever_report: dict | None = Field(
        default=None,
        description="L-1: 정본 4-레버 산정 보고서(LeverReport.model_dump()). "
                    "case == 'LEVER' 일 때만 존재.",
    )
...
class SolutionPlan(BaseModel):
    ...
    # ── 목표 매출 근거 (Phase β/γ) ──
    # 세 필드 모두 optional. β 이전 세션 JSON 도 그대로 검증 통과.
    tier_plan: TierPlan | None = Field(
        default=None,
        description="3/6/12개월 tier 분할. A/B/C 케이스만 존재 (D/E 는 None).",
    )
    tam_meta: TamMeta | None = Field(
        default=None,
        description="상권 TAM 조회 결과. address 지정 + 시도한 경우만 dict.",
    )
    target_meta: TargetMeta | None = Field(
        default=None,
        description="목표 매출 산정 근거 메타 (근거 섹션용).",
    )
