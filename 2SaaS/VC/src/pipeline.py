...
from src.generator.rationale_xlsx import write_rationale_xlsx
...
def run_pipeline(
    ...
    Args:
        raw_json: 스크래퍼 raw JSON (옵션). 제공 시 hallucination 검수 활성화.
    ...
    # raw solution dict (tier_plan/tam_meta/target_meta 포함 — xlsx 근거 시트용)
    solution_raw = json.loads(solution_json.read_text(encoding="utf-8"))

    # raw 데이터 로드 (옵션)
    raw_data: dict | None = None
    if raw_json is not None:
        raw_data = json.loads(Path(raw_json).read_text(encoding="utf-8"))
    ...
    build_menu_xlsx(menu_plan, xlsx_path, solution_plan=solution_raw)
    build_solution_docx(solution_plan, docx_path)

    # 3-1. 담당자 내부용 산정 근거 XLSX (업주 제공 파일 아님)
    #   담당자 피드백(2026-04-22): 기존 txt 버전 가독성 부족 → XLSX 4시트 구성으로 전환.
    #   시트: 1.요약 / 2.레버분석 / 3.가정·한계 / 4.REF검증
    rationale_path: Path | None = output_dir / f"{store_name}_목표매출_산정근거.xlsx"
    try:
        write_rationale_xlsx(solution_plan, rationale_path)
    except Exception as e:  # 산정 근거 생성 실패는 파이프라인 전체를 막지 않음
        print(
            f"[WARN] 산정 근거 XLSX 생성 실패 ({type(e).__name__}: {e})",
            file=sys.stderr,
        )
        rationale_path = None

    # 4. 자동 검수 (raw 있으면 hallucination 검수도 추가)
    report = validate_all(
        menu_plan, solution_plan, xlsx_path, docx_path, raw_data=raw_data
    )
...
def _format_console_report(result: PipelineResult) -> str:
    ...
    if result.rationale_path is not None and result.rationale_path.exists():
        lines.append(
            f"  [담당자 내부용] 산정 근거: {result.rationale_path} "
            f"({result.rationale_path.stat().st_size:,} bytes)"
        )
    lines.append("")
    lines.append(result.report.detail_report())
    return "\n".join(lines)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="raw JSON 경로 (옵션, hallucination 검수 활성화)",
    )
    parser.add_argument(
        "--save-report",
        type=Path,
    ...
    try:
        result = run_pipeline(
            args.menu, args.solution, args.output_dir, raw_json=args.raw
        )
