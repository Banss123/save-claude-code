...
REVIEW_FONT = Font(name=FONT_NAME, size=FONT_SIZE, bold=True, color="C00000")


def _write_tier_meta_sheet(ws, solution_plan: dict) -> None:
    """목표매출 근거 시트 작성.

    solution_plan 의 tier_plan/tam_meta/target_meta 를 행 단위로 기술.
    tier_plan=None 이면 단일 케이스 라벨 행만, tam_meta=None 이면 '미연동' 표기.
    """
    # 열 너비
    ws.column_dimensions["A"].width = 1.0
    ws.column_dimensions["B"].width = 28.0
    ws.column_dimensions["C"].width = 48.0

    target_meta = solution_plan.get("target_meta") or {}
    tier_plan = solution_plan.get("tier_plan")
    tam_meta = solution_plan.get("tam_meta")

    # 1행 타이틀
    title = ws.cell(row=1, column=2, value="목표 매출 산정 근거")
    title.font = TITLE_FONT
    title.fill = TITLE_FILL_PROPOSED
    title.alignment = CENTER_ALIGN
    ws.merge_cells(start_row=1, start_column=2, end_row=1, end_column=3)
    ws.cell(row=1, column=3).fill = TITLE_FILL_PROPOSED
    ws.row_dimensions[1].height = ROW_HEIGHT

    # 2행 헤더
    for col_num, text in [(2, "항목"), (3, "값")]:
        c = ws.cell(row=2, column=col_num, value=text)
        c.font = HEADER_FONT
        c.fill = HEADER_FILL
        c.alignment = CENTER_ALIGN
        c.border = THIN_BORDER
    ...
    def _add_row(label: str, value: str, *, bold_value: bool = False) -> None:
        nonlocal row
        lc = ws.cell(row=row, column=2, value=label)
        lc.font = GROUP_FONT
        lc.fill = GROUP_FILL
        lc.alignment = LEFT_TOP_ALIGN
        lc.border = THIN_BORDER
        vc = ws.cell(row=row, column=3, value=value)
        vc.font = MENU_FONT_BOLD if bold_value else MENU_FONT
        vc.alignment = LEFT_TOP_ALIGN
        vc.border = THIN_BORDER
        ws.row_dimensions[row].height = ROW_HEIGHT
        row += 1

    # ── 케이스 ──
    case = target_meta.get("target_case")
    case_label = target_meta.get("target_case_label")
    if case and case_label:
        _add_row("케이스", f"{case} · {case_label}", bold_value=True)
    elif case_label:
        _add_row("케이스", case_label, bold_value=True)

    # ── tier_plan ──
    if tier_plan:
        # 기본 배수: 12개월차 (최종 스트레치)를 기본 배수로 표기
        tier3 = tier_plan.get("tier3_12m") or {}
        if tier3.get("multiplier") is not None:
            _add_row("기본 배수", f"{tier3['multiplier']:.2f}배")
        # 3/6/12개월 목표
        for key in ("tier1_3m", "tier2_6m", "tier3_12m"):
            tier = tier_plan.get(key)
            if not tier:
                continue
            target = tier.get("target", 0)
            _add_row(TIER_LABELS[key], f"{int(target):,}원")
    else:
        # 단일 행 (D/E 케이스 또는 tier 미제공)
        _add_row("tier 분할", "해당 없음 (단일 목표만 적용)")

    # ── 산정 근거 ──
    rationale = target_meta.get("target_rationale") or "—"
    _add_row("산정 근거", rationale)

    # ── TAM 상태 ──
    if tam_meta is None:
        _add_row("TAM 상태", "미연동")
    else:
        available = bool(tam_meta.get("available", False))
        if available:
            tam_won = tam_meta.get("tam_monthly_revenue_won")
            if tam_won is not None:
                _add_row("TAM 상태", f"연동됨 (월 추정 {int(tam_won):,}원)")
            else:
                _add_row("TAM 상태", "연동됨")
        else:
            reason = tam_meta.get("reason") or "사유 미상"
            _add_row("TAM 상태", f"미연동 — {reason}")

    # ── 담당자 검토 경고 (빨간 볼드) ──
    lc = ws.cell(row=row, column=2, value="담당자 검토")
    lc.font = GROUP_FONT
    lc.fill = GROUP_FILL
    lc.alignment = LEFT_TOP_ALIGN
    lc.border = THIN_BORDER
    vc = ws.cell(row=row, column=3, value="필수")
    vc.font = REVIEW_FONT
    vc.alignment = LEFT_TOP_ALIGN
    vc.border = THIN_BORDER
    ws.row_dimensions[row].height = ROW_HEIGHT
...
def build_menu_xlsx(
    ...
    """MenuPlan → XLSX 문서를 생성합니다 (업주 제공용 — 현안/가안 2시트).

    Args:
        solution_plan: SolutionPlan dict (옵션, 시그니처 호환용).
            담당자 피드백(2026-04-22): 업주 제공 XLSX 에는 산정 근거를 싣지 않는다.
            산정 근거는 별도 텍스트 파일(``src.generator.rationale_text``)로 분리.
            ``_write_tier_meta_sheet`` 헬퍼는 향후 내부 XLSX 토글 대비해 유지.
    """
    ...
    # 참고: solution_plan 은 시그니처 호환 위해 유지하되,
    # 업주 제공 파일이므로 산정 근거 시트를 포함하지 않는다.
