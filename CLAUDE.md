...
## 신규 머신 셋업 (Windows → Mac 이전 직후)

이 ~/.claude 디렉토리를 새 머신에 clone한 직후라면 `SETUP_MAC.md` (Mac) 를 먼저 읽고 10단계 셋업을 실행한다. 시크릿 파일·MCP 설정·피그마 소켓 자동 시작·심볼릭 링크 복구가 모두 거기에 정리되어 있다.
